...
os.system("seqkit split2 ribo-out/ribo-out.ribodbmaker.final.fa -s 10 -f -o split") 
from subprocess import Popen, PIPE
#f_list = glob.glob('ribo-out/ribo-out.ribodbmaker.final.fa.split/ribo-out.ribodbmaker.final.part_*')
f_list = glob.glob(os.path.join("ribo-out/ribo-out.ribodbmaker.final.fa.split/", "ribo-out.ribodbmaker.final.part_*"))
#cmscan-df = 'cmscan --cpu 16 --mid -T 20 --verbose --tblout tblout.df.txt rrna.cm > /dev/null'
cmd1 = "cmscan --cpu 16 --mid -T 20 --verbose --tblout tblout.df.txt rrna.cm ribo-out/ribo-out.ribodbmaker.final.fa.split/gfile > /dev/null"
cmd2 = "cmscan --cpu 16 --mid -T 20 --verbose --anytrunc --tblout tblout.at.txt rrna.cm ribo-out/ribo-out.ribodbmaker.final.fa.split/gfile > /dev/null"
for gfile in f_list:
    print("GFILE", gfile)
    #subprocess.Popen(args=["cmscan", "--mid", "-T 20", "--verbose", "--tblout", "tblout.df.txt", "rrna.cm", gfile, "> /dev/null"], shell=True)
    #subprocess.Popen(args=["cmscan --mid -T 20 --verbose --tblout tblout.df.txt rrna.cm", gfile, "> /dev/null"], shell=True)
    os.system("cmscan --cpu 16 --mid -T 20 --verbose --tblout tblout.df.txt rrna.cm f_list > /dev/null&")
    #os.system("cmscan --cpu 16 --mid -T 20 --verbose --anytrunc --tblout tblout.at.txt rrna.cm gfile > /dev/null&")
    #subprocess.run(args="cmd1", shell=False)
    #subprocess.wait
    #subprocess.Popen(args=["cmscan", "--mid", "-T 20", "--verbose", "--anytrunc", "--tblout", "tblout.at.txt", "rrna.cm", gfile, "> /dev/null"], shell=True)
    #subprocess.run(args="cmd2", shell=False)
    #subprocess.wait
