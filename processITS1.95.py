...
#Run CMscan using the output of ribodbmaker.pl
    #print('cmscan1')
    os.system("cmscan --mid -T 20 --verbose --tblout tblout.df.txt rrna.cm > /dev/null &")
    #print('cmscan2')
    os.system("cmscan --mid -T 20 --verbose --anytrunc --tblout tblout.at.txt rrna.cm > /dev/null")
    return cmscan_output
with open('ribo-out/ribo-out.ribodbmaker.final.fa') as fd, Pool() as pool: 
    cmscan_output = pool.map(run_cmscan, fd, chunksize=5,)
