# -*- coding: utf-8 -*-
"""
Created on Wed Sep 13 13:59:13 2023

@author: mcveigh

revisision of processGiardiaITS1.93.py to better handle reverse complemented sequences
any sequences with 5.8S rRNA on the minus strand are reverse complemented. Rerun cmscan on these
and merge the results. Both complete and partial 5.8S rRNA and considered for inclusion due excessuve repeats in Giardia
...
FiveBothPartialMinus = CMscan_df[(CMscan_df['trunc'] == "5'&3'") & (CMscan_df['strand'] != "+")]
FiveBothStrongMinus = FiveBothPartialMinus[(FiveBothPartialMinus['score'] > 30) & (FiveBothPartialMinus['mdl_from'] < 15) & (FiveBothPartialMinus['mdl_to'] > 107) & (FiveBothPartialMinus['strand'] != "+")]
...
for seq_record in SeqIO.parse("ribo-out/ribo-out.ribodbmaker.full.fa", "fasta"): 
    ...
    if seq_record.id in FiveCompleteMinus['accession'].tolist() or FiveBothStrongMinus['accession'].tolist():
...
FiveBothPartial = combined_df.loc[combined_df['trunc'] == "5'&3'"]
FiveBothStrong = FiveBothPartial[(FiveBothPartial['score'] > 30) & (FiveBothPartial['mdl_from'] < 15) & (FiveBothPartial['mdl_to'] > 107)]
...
for seq_record in SeqIO.parse("plus_strand_seqs", "fasta"): 
    ...
    if seq_record.id in Fivecomplete['accession'].tolist():
        ...
        if seq_record.id in SSU_RNA_df['accession'].tolist():
            if seq_record.id in Fivecomplete['accession'].tolist() or FiveBothStrong['accession'].tolist():
                if SSUend > fivestart:
