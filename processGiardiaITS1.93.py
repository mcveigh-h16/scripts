...
print('cmscan1')
os.system("cmscan --cpu 24 --mid -T 20 --verbose --tblout tblout.df.txt rrna.cm ribo-out/ribo-out.ribodbmaker.final.fa > /dev/null")
print('cmscan2')
os.system("cmscan --cpu 24 --mid -T 20 --verbose --anytrunc --tblout tblout.at.txt rrna.cm ribo-out/ribo-out.ribodbmaker.final.fa > /dev/null")
...
Five_RNA_df['score'] = pd.to_numeric(Five_RNA_df['score'])
Five_RNA_df['mdl_to'] = pd.to_numeric(Five_RNA_df['mdl_to'])
Five_RNA_df['mdl_from'] = pd.to_numeric(Five_RNA_df['mdl_from'])
...
for seq_record in SeqIO.parse("ribo-out/ribo-out.ribodbmaker.final.fa", "fasta"): 
    ...
    if seq_record.id in FiveCompleteStrongHit['accession'].tolist() or FiveBothStrong['accession'].tolist():
